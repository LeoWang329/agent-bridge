#!/usr/bin/env node
// Agent Loop — 可视化 SSE 服务器(零依赖,仅 Node 内建)
// 拷自 skills/agent-bridge-roundtable/viz/serve.mjs(同版语义,仅改标识行:LOOP_URL / [loop-viz] / 用法文案);
// 修 bug 时两份同步。同步基准:圆桌 viz 交互对齐后的 2026-07-06 状态(比对:除标识行外 diff 应为空)。
//
// 用法:  node serve.mjs <run-dir> [--port N]
//   <run-dir>  一次 loop 运行的目录(含 transcript.jsonl),见 EVENTS.md
...
//           始终无客户端满 10 分钟 → 退出。三重保险的第一重(另有 viz.pid + 主控显式 kill)。
//   - 启动成功打印一行  LOOP_URL=http://127.0.0.1:<port>  供主控抓取。
//   - 服务器只读,绝不写 transcript.jsonl(真理源单一 writer = 主 agent)。
...
function die(msg) { process.stderr.write(`[loop-viz] ${msg}\n`); process.exit(1); }
...
function poll() {
  ...
  catch { return; } // 文件还没出现(主控应先写 run:started 再起服务),下次再看
...
function shutdown(reason) {
  ...
  process.stderr.write(`[loop-viz] 退出(${reason})\n`);
  process.exit(0);
}
...
function listen(port, isRetry) {
  ...
  server.listen(port, "127.0.0.1", () => {
    ...
    // 唯一约定输出:主控 grep 这一行拿 URL
    process.stdout.write(`LOOP_URL=http://127.0.0.1:${actual}\n`);
    poll();                          // 立刻吃掉已有内容
