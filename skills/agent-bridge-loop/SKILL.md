...
2. **先加载桥 skill 拿完整用法**:本流程建在 agent-bridge 之上。**先加载 `agent-bridge` skill**(手动 link 态名为
   ...
   `doctor`/`--list-models` 探测、`append_system_prompt_file` 注入角色等。
3. **两条致命纪律(内联,别只靠跨引用)**:
...
- AC4 [review] 异引擎 reviewer 对 base..head 给 APPROVE   ← 关键 goal 才写(可选)
边界: 不动 <Y>
## Goal G2: …(默认按序依赖)
## 全局约束
```
...
| 代码评审 | 收官整支 broad review(必过)+ 关键 goal 可选 `[review]` AC(你代执行、lazy) |
