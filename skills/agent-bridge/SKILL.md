...
2. `agent_bridge_wait` 传**短 `timeout_ms`**(如 `300000` = 5 分钟 / `600000` = 10 分钟)阻塞等：
   ...
   - **到点还没完** → 返回 `{ timedOut:true, settled, pending }`,**不报错**。这时你可以:看一眼 `agent_bridge_status`/`agent_bridge_result` 的当前进展、回报用户、去干别的,然后**再 `wait` 一次**继续等。
...
- 公共核心字段：`id`、`agent`、`owner`、`cwd`、`write`、`model`、`effort`、`status`、`pid`、`createdAt`、`updatedAt`、`lastError`、`logFile`。（`status` 已表达是否在跑，不再单列 `isStreaming`。）
...
- `recentEvents`（`status`/`result` 带）：精简生命周期串(turn/工具/状态/错误),已**过滤逐 token、消息边界、心跳等噪声**(`message_update`/`*delta`/`message_start`/`message_end`/`extension_ui_request`/`rateLimits`/`tokenUsage`)并合并连续同类。要全量原始事件看 `logFile` 或 UI 的 SSE 流。`wait` 的每条结果不带 `logFile`(需要日志路径用 `status`/`result`)。
