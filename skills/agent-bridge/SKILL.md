...
```
open_session(agent, cwd, access)           →  拿 session_id
...
有一个**只读本地网页**能把这次运行的委托会话与每一轮画出来（排查「卡在哪了 / 那轮到底说了什么」）。

⚠️ **默认关，而且开关只在 MCP server 启动那一刻读一次。** 于是两条：改了要**重启客户端**；
**出问题那次当时没开就没有记录**，事后补不了。所以要在**做重度委托 / 多 agent 编排之前**就打开。
代价：开着等于「本机磁盘上存在全部委托原文」——桥平时的日志刻意不落 prompt 全文，它会落。
**敏感内容别开。**

- 用户问起观测台、或要开 / 要打开页面 → **先读 `<本 skill 目录>/viz/USAGE.md`**（开法、找目录、
  起 viewer、隐私代价、排查全在那儿），别凭印象答。
- 一句话版：给**这个 MCP server 进程**加环境变量 `AGENT_BRIDGE_VIZ=on`（怎么加看你用的客户端——
  多数客户端的 server 配置里有 `env` 字段；也可以设成系统级变量，server 从客户端继承），
  重启客户端；然后在临时目录里找 `agent-bridge-viz-*`，
...
**任务能切成相互独立的子任务时，优先开多个会话并行跑**——能显著压缩总时长。前提是按真正独立的边界切分，并守住下面的并发纪律（`access:"write"` 会话别撞文件、同会话别并发 `send`）。
...
> - **`write` 会话别撞文件**：任一 `access:"write"` 会话运行期间，别让另一个 `write` 会话**或主 agent 自己**改同一批文件。无法保证不重叠就串行：第一个完成 → `git diff` → 把改动作为上下文写进第二个的 prompt。
...
**effort 量体裁衣（按难度/时延，不为省钱）：** 重活（复杂实施/评审）用 `xhigh`；改小范围、跑定位用 `medium` 只是因为低 effort 更快、够用即可，**不是为省 token**。拿不准就往高了给。
