...
2. 用返回的 `session_id` 调用 `agent_bridge_send_message`（**默认阻塞**：等本轮结束并直接返回结果）。
...
要让多个 agent **并行**干活，不要逐个阻塞等：

> `agent_bridge_send_message` **默认阻塞**（`wait:true`，直接返回结果）。并行时要**显式**传 `wait:false` 走非阻塞。

1. 对每个会话 `agent_bridge_send_message` 显式传 `wait:false`，立刻拿到 ack，任务在后台跑。
2. 再用**一次** `agent_bridge_wait` 阻塞等结果，省去循环轮询 `agent_bridge_status`：
...
> 串行委托（派一个 → 等结果 → 再派下一个）直接用 `agent_bridge_send_message`（默认阻塞，直接返回结果）即可，不需要 `agent_bridge_wait`。`wait` 与 `wait:false` 是给“并行 + 收口”用的。
