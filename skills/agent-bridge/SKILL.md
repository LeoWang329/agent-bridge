...
要让多个 agent **并行**干活，不要逐个 `wait:true` 阻塞等：

1. 对每个会话 `agent_bridge_send_message` 传 `wait:false`（默认），立刻拿到 ack，任务在后台跑。
...
> 串行委托（派一个 → 等结果 → 再派下一个）直接用 `agent_bridge_send_message` 的 `wait:true` 即可，不需要 `agent_bridge_wait`。`wait` 是给“并行 + 收口”用的。
