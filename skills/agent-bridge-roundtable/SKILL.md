...
- **收尾三查**:`agent_bridge_status` 会话已空、`git worktree list` 无残留、`viz.pid` 已处理(自灭看门狗是兜底,不是免检理由)。
