...
### 可选拓扑:熟路题的「1+2 作者-批评家」(非默认,先过上面这道闸)
...
2. **判不准 → 用盲发圆桌**(不是「按熟路起步」)。盲发 R0 的独立信号是本 skill 的核心价值,不确定时不能拿它冒险——宁可多花一半成本保住独立性。**升级便宜、降级浪费**:批评家若炸出「方案根本方向存疑」(路线错、非细节错),把作者全案当 `rounds/p1-r0.md`、另开 2 席补盲发 R0(即回到 §整体流程 step 3 的盲发圆桌),无需推倒重来。
3. **选作者席别纠结「谁最强」**。作者引擎强弱不决定终质(批评家会兜底);**默认选 `contextUsage` 最低的席**(与 §失败模式「上下文腐化」联动,把最重的产全案任务放在最空的席),或领域最匹配的席。
4. **升级逻辑替换**:1+2 的 R0 只有单一作者,**没有「全席一致 needs-human」可判**(§发言顺序 分歧分类)。改为:**任一批评家**独立把某点标 needs-human → 直接进升级队列(批评家视角本就偏怀疑,单席即足以触发)。
...
- **主席**:你。调度、策展简报、判收敛、指派红队、人在环升级、综合终稿、**写 `transcript.jsonl`**。
  - **收敛性亲自判、终稿亲自写(纪律重申,非新规)**:判收敛(写 `schedule:decision`)本就是主席职责——你有全场最全上下文、终稿也是你写的。把终裁**外包给某席**并不增加中立性(综合权仍在你手),只多一个重轮次、还常落在高占用席上(本场终裁落在 317k→464k 的席)。主席「中立」指**不下场持某方案立场**,不是放弃裁判职能;席位的职责是论证与互攻,不是替你判收敛。
...
2. [提示用户] 一次性问「要开可视化实时看进展吗?(默认否)」→ 同意才起服务(§可视化)。
   建 run-dir + 写 .gitignore 一行 + append run:started(**若选了 1+2 作者-批评家拓扑,`run:started` 带 `topology:"author-critics"`**,默认盲发可省;见 §可选拓扑 / EVENTS.md)。
3. Round 0 盲发并行:**中立框设写进 `briefs/r0-framing.md`(一处真源,避免同文 N 贴、改框设只改一处)** → 每席 open_session(`access` 按 §席位权限:默认 `read` / 改代码 `write`+worktree, 注入 panelist.md) → 逐个非阻塞 send:消息只含**席位号 + 「读 `briefs/r0-framing.md` 后独立作答」**(`read` 席读 run-dir 内相对路径;**write 席读不到 run-dir**——主席先把框设 `cp` 进该席 `<worktree>/.rt-inbox/briefs/r0-framing.md`,消息里给其绝对路径。框设很短也可直接 inline,不强制走文件)
...
- **全席一致 needs-human**(每席都在各自「需人拍板」小节独立标注同一项)→ 该分歧标 `kind:"needs-human"`,**跳过对抗轮直接进升级队列**(§人在环)。全体独立同判是强信号,对抗轮翻案概率低,再跑一轮多半在重推已知结论。若仍想给用户配「决策证据包」(如红队失效场景),由主席自己写进升级选项的**附注**,不值一个席位轮。
- **立场分裂 / 有席声称可技术裁决 / 有席未表态** → 才开对抗轮(steelman + 红队)测真收敛 vs 礼貌投降。
- (**1+2 作者-批评家拓扑**下 R0 只有单一作者,不存在「全席一致」——改为**任一批评家**独立把某点标 needs-human 即进升级队列,见 §可选拓扑。)
...
- **轻任务折叠,不单独成轮**。预计只产一两段、不需新调查的任务(如「读完对方论证后表态是否翻转」)折叠进该席同轮相邻任务,一次消息一次收口。判据:任务书只有一个动词、不需打开新文件 → 折叠。**但折叠的是轮次编排、不是机制**:steelman-翻转测试仍是反假共识核心(§反谄媚),折叠后的 steelman 段须有**独立小节标题、质量不低于原独轮**(主席收口时检查),否则退回独立轮。
- **匿名不加码**(防误伤、非新规):文风泄漏是弱信号 × 弱效应(§信息传递「残余诚实交代」),**不为它引入改写/摘要层**——那会重新引入内容失真,正是本设计要根除的。
...
- **建分歧索引 + 框任务**:写 `briefs/rN-brief.md`——每个本轮该咬的分歧一行**中立标签** + **指向相关 `rounds/` 全文的指针**(立场 A → `rounds/p2-r0.md`、立场 B → `rounds/p3-r0.md`)+ 明确任务(先 steelman 对方最强形式,再反驳/收敛/标真取舍)。**主席只索引、不复述实质**——实质由匿名全文承载,主席**无从曲解**。小索引可直接 inline 进 `send_message` 的 message,大了才落 `briefs/` 文件。收敛轮简报还须含一节**共识靶单**(上一轮无人反对的每条技术判断列成表,供红队席逐条攻,见 §反谄媚);共识靶单与分歧索引**分开列、别混**。
- **coverage 记账(治「无声漏点」)**:append 一条 `coverage` 事件,把上一轮**每席**发言标 `included`/`parked`/`irrelevant` + 一句理由。让「主席漏掉某席整条视角」变成**可审计的显式决策**(红队/人可质问「凭什么把 X 判无关」)。**记账 ≠ 被审**:coverage 是主席自报账,真闭环靠**席位反审**——收敛轮席位读简报第一件事是核对主席的分歧索引/共识靶单有没有漏掉或曲解自己上轮的关键论点,有就在发言开头以固定标记 `[COVERAGE-ERR]` 起头指出(见 `roles/panelist.md`),便于事后 grep 与追责。
...
| **上下文腐化** | 收敛轮**分派前**先看各席 `contextUsage.tokens`:**≥300k 的席不派重读任务**(读多份全文、终裁类),重任务给**最低占用席**;某席 **≥400k** → 关旧开新、交接摘要塞进新会话 initial_prompt(见桥 skill)。「反正是最后一轮」不是豁免理由。(跨后端读数口径**已对齐**——claude 于 `12c3bfd` 修为末次 per-call input 侧、不再虚高;仍有分词器 10–30% 差异,当粗粒度触发器足够,直接按 `tokens` 比,**不要再对半折算**。) |
