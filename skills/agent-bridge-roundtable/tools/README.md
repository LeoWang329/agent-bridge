...
- `--agent` 取本场后端(当前:`omp` / `codex` / `claude` / `cursor` / `kimi`);`--model` **可选**——省略时 `turn:produced.payload.model` 记为 `null`(后端用默认模型)。
  ...
  - **kimi 席**:内置黑名单**已含** `kimi` 与厂商名 `moonshot`,一般无需 `--extra-names`;但 `k2`/`k3` 这类**短代号故意不进全局表**(太短、易在正文/版本号里误伤)——议题里确实要扫它们时逐席用 `--extra-names` 带上。
