...
> **谁写**:只有主席(主 agent)写 `transcript.jsonl`、`rounds/`、`briefs/`——且 `rounds/` 是 `cp` 桥落好的 `textRef`(字节直传,不转写)。参会席默认 `access:"read"`(读+执行,无 Edit/Write;**cursor / kimi 例外**——原生写工具仍在、靠角色纪律不写),改代码的 write 席只在自己的 git worktree `.roundtable/wt/<rtId>/seat-<x>/` 里改(写硬隔离,读非硬:桥无 OS 沙箱,靠角色纪律不去遍历真相源),**都不写真理源**。
...
|---|---|---|
| `run:started` | `question`,`seats:[{seat,agent,model,role}]`,`maxRounds`,`budget`,`vizEnabled`,`mode?`,`modeRationale?`,`topology?` | 圆桌开场:议题、阵容、上界。`mode`(可选)= `"adjudicate"`(裁决,缺省)\|`"explore"`(探索,启发轮;见 SKILL §两种模式 + `modes/` 手册);`modeRationale`(可选)= 主席选模一句话理由;`topology`(可选,仅裁决模式)= `"blind-panel"`(默认盲发圆桌)\|`"author-critics"`(熟路题 1+2 作者-批评家,见 `modes/adjudicate.md`);缺省视同 `blind-panel`。viz 对未知字段/未知值忽略即可 |
...
| `disagreement` | `topic`,`positions:[{label:"立场A",stance,fromSeat?}]`,`kind:"by-merit"\|"needs-human"\|"fork-parked"` | 一个分歧点:立场 + 能否靠论证解决。`fork-parked`(仅探索模式)= 启发轮暴露的互斥岔路,按模式纪律**场内不裁**——升级问人或终稿标注、建议另开裁决场(见 `modes/explore.md`)。每条立场应带 `fromSeat`(供审计 + **观察台对人类可见**:把立场映射回某席),但**对席匿名**——不写进 `briefs/`,别席永远见不到「谁持哪个立场」。匿名约束的是席位彼此、不含人类观察者。`fromSeat` 可缺省(半写行/主席未标),缺时观察台略过该来源徽标、不报错 |
...
<cwd>/.roundtable/
  rt-<id>/                     ← 一场圆桌的 run-dir(真理源都在这)
    ...
    briefs/r<N>-brief.md       ← 主席每轮的索引(裁决=分歧索引+共识靶单+红队任务,探索=素材索引;见 modes/ 手册)+ 指向 rounds/ 全文(参会席 read 这个 + 按需读 rounds/;不读 transcript)
