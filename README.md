...
3. **`wait`** to collect results — this is the ONLY way a result reaches you; the bridge never pushes one, so an uncollected turn is simply lost. `mode: "any"` (the default) returns on the first one to finish and hands you the rest in `pending`; `mode: "all"` blocks until every session finishes. `timeout_ms` defaults to 10 minutes (`AGENT_BRIDGE_JOIN_TIMEOUT_MS`) and timing out is non-destructive — the turn keeps running, so just call again while `pending` is non-empty. On timeout it returns `pendingSnapshots` (per still-running session: `status`, `charCount`, a `tail` of the live partial output — capped by `tail_chars`, default 240 — and the latest lifecycle event) so progress is visible without a separate `status` call. This is how you fan out across parallel sessions.
4. Reuse the same `session_id` for follow-ups.
5. **`close_session`** when done — omit `session_id` to close **every** session this server manages (a bulk-cleanup fallback after a crash or forgotten close). Closing a session whose turn is still running destroys that turn's output for good, so it is **refused by default**: you get `{blocked:true, runningSessionIds}` and nothing is closed (the bulk form is atomic — one running session blocks them all). Collect or `abort` first, or pass `force:true` if you really mean to discard the work.
