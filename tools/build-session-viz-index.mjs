 ...
 *
 * 设计稿更新时重跑本脚本即可：
 *   node tools/build-session-viz-index.mjs <设计稿.html>
 */
...
// 产物落在 skill 里(那是运行时资产);本脚本住在 tools/ 里(它是开发工具,跑 skill 时用不到)。
const OUT = path.join(HERE, "..", "skills", "agent-bridge", "viz", "index.html");
const SRC = process.argv[2];
if (!SRC) { console.error("用法: node tools/build-session-viz-index.mjs <设计稿.html>"); process.exit(2); }
...
// ── 1c. 右栏空态拆成两种（「还没有」≠「有但你没选」） ────────────────────────
//
// ⚠️ 设计稿的 `renderDetail()` 里 `if(!s)` 一个分支管两件事：
//    「这次运行一个会话都没有」和「有会话但当前没选中」。
//    共用一句话的后果在真页面上一眼可见：左栏明明列着卡片、右栏却说
//    **「还没有可查看的会话，主 agent 一旦把任务派出去，左侧会出现会话卡片」**——
//    页面自己跟自己打架，而用户能得出的唯一结论是"它坏了"。
//    （这和写的那一侧「没写过快照」被当成「读不出来」是同一类：
//      两件不同的事走同一个分支，说出来的话就必然对其中一件是假的。）
{
  const NONE = `'<h2>还没有可查看的会话</h2><p>主 agent 一旦把任务派出去，左侧会出现会话卡片，选中即可在这里逐轮查看它问了什么、答了什么。</p></div>'`;
  if (!html.includes(NONE)) throw new Error("找不到右栏空态文案，设计稿结构变了");
  const PICK = `'<h2>左边选一个会话</h2><p>这次运行已经有 '+SESSIONS.length+' 个会话。点开左侧任意一张卡片，就能在这里逐轮查看它问了什么、答了什么。</p></div>'`;
...
/* ---- 三种"看不到内容"必须严格区分（docs/STATE-session-viz.md §6.2）----
   混成一句话，页面就会做错事：
