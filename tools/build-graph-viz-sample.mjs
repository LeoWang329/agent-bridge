/**
 * 生成 `skills/agent-bridge-graph/viz/sample/` 下那三份**冻结样例** transcript。
 *
 ...
 * run:final 恒等式),而那些不变式正是 `tests/test-viz-graph.mjs` 要断言的东西。
 *
 *   node tools/build-graph-viz-sample.mjs
 *
 ...
 *    错误会一路藏到真跑那天。`tests/test-viz-graph.mjs` 把这四条做成了断言。
 *
...
// 冻结样例落在 skill 的 viz/sample/ 下(SKILL.md 教用户直接打开它看观测台长什么样);
// 本脚本住在 tools/ 里 —— 它只在改样例时跑一次,跑 skill 时用不到。
const SAMPLE = path.join(HERE, "..", "skills", "agent-bridge-graph", "viz", "sample");
...
fs.rmSync(path.join(SAMPLE, "nodes"), { recursive: true, force: true });
...
for (const { graphId, text } of outs) {
  ...
  const p = path.join(SAMPLE, "nodes", ".runs", graphId, "transcript.jsonl");
