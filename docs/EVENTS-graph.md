...
⚠️ **第 3 条曾经写反过,而且反得很贵。** 原文是「只认相对形式;绝对路径不匹配——跨 out-dir 的引用不是本图的边」。
那句话的**意图**没错(别把指向别的 out-dir 的引用画成本图的边),但「是不是绝对路径」是个**很差的代理**:
`runNode` 回执里的 `artifactPath` **就是绝对路径**,而「把上一个环节的 `artifactPath` 拼进下一个环节的提问」
正是这套 API 最标准的串联写法(skill 自带的 `examples/hetero-audit.mjs` 就是这么写的)。
于是这条规则**恰好把 API 自己给出的东西挡在门外**:实测一次四环节真跑,汇总环节的提问里明写着前三个环节产出的完整路径,
推断出来的边是 **0 条**,页面据此把图视图整个禁用掉——这个功能在它的**头号用法**上是失效的。

⚠️ **为什么改成"都认"而不是"解析出来落在本次 outDir 里才认"**:后者看着更精确,代价是要在 Windows 上比对路径
(短名 `C:\Users\LEO~1.WAN\…` 对长名 `C:\Users\leo.wang\…`、大小写、正反斜杠)。这种比对**对不上时是往关的方向失败** ——
推断边又回到 0 条,而"0 条"和今天的常态长得一模一样,没人会发现。**一道会悄悄退回原 bug 的闸,比不加更糟。**
真正的防线在下游而且已经有了:§10.7 规定端点 `id` 必须在本 graph 里恰好出现 1 次才画边。
所以外来路径要造成误导,得它的 `<id>` **恰好跟本图某个环节重名**——很窄;而且那条边是虚线、标着"这是猜的"、点开有依据原文。
...
- ⚠️ **但"连不上要说出来"这一句只对 A 档成立,B 档必须闭嘴。** 声明是**用户写下的一句话**,连不上多半是把 id 打错了,
  不说他就只看到「我明明写了依赖,图上却没有」;推断是**我们自己猜的**,猜空了就是没猜中,**页面没有立场为自己的猜测报警**。
  照 A 档处置 B 档,用户会看到一条**他从没写过的依赖**被报成"连不上"——尤其在他一条 `deps` 都没声明的时候,那句话无从理解。
  (第 3 条改成"绝对路径也认"之后这更要紧:扫得多了,猜到别的 out-dir 的 `id` 也随之变多。)
