...
| **验证 LLM 成本纪律:预算入合同,花法可优化、判准不可缩**(补 2026-07-08,用户要求) | 验证期 LLM 调用是**乘法成本**(n 采样 × K 自检 × 迭代数 × 全量重验),不设防烧穿;与桥「不考虑 token/成本」不矛盾——那条讲**编排取舍**不缩水,这条管**测试期浪费**(文内明说,防"上层原则误导"复发)。预算与 n/m 同属**合同**(验证者不发明预算,同"不替出题人拍阈值");省钱手段全部数学/工程等价:early-stop(成功 ≥m 停 / 失败 > n−m 停)、先小后全(中间轮 fail-fast 小样本,**PASS 定论轮跑满全量**——验收语义不变)、判次落盘复用、judge 按判别难度选量级、稳定性自检小样本夹具化。唯一禁区 = 私自缩采样(降验收标准);耗尽走 BLOCKED/NOTES 升级(零新事件) |
| **规划者(planner)角色:重文书起草剥离主控**(2026-07-08,用户提案) | 主控上下文膨胀大头 = 「读一堆、想很久、写一份」的文书活(起草合同最甚,用户实测主控降智);剥给 planner 后主控回归「用户通道+桥工具+记账笔+闸门」= 演化文档 L3 模型关节清单(路线自洽)。附带治理增益:**起草者 ≠ 裁判**(此前主控又写合同又仲裁合同争议,同尺子所有权逻辑)。**编排不可剥**:桥 MCP 工具只在主控会话,委托 agent 指挥不了别人——planner 不是也不能是"编排者的编排者",调度的接盘者是 L3 的代码 |
| **工作台 = run-dir + `planning/` 自留地;星型拓扑不破**(2026-07-08,用户提案) | 用户提议"统一文件路径交互免传话"——工作台其实已存在(状态外置红利):`iterations/` 归档 planner 直读**零传话**。补 `planning/`(同 validation/ 先例:每角色一块自留地、每块地单一 writer,归因清晰)。**建议/修约仍过主控**:那一跳是闸不是传话——合同是生成者唯一需求来源,直通 = 第二个老板 + 账本外指令;载荷走磁盘、消息只走指针,主控上下文只碰结论。真理源在 gitignore 下洁净树审计照不到 → planner 越界写真理源靠红线 + 主控只认自己 cp 的版本 |
| **主控自身上下文卫生**(2026-07-08) | 生成者 400k 关旧开新早有,主控自己没有——但状态全外置本就意味着主控可被替换:临近腐化(遗漏协议步骤/重复劳动)→ 收尾当前迭代、compact/新开会话从 run-dir 接手(合同+transcript 尾+git log+goal 指针)。planner 剥重活治的是"长得快",这条治的是"长满了怎么办" |
| **双评修复(codex + deepseek 各 NEEDS_FIXES,2026-07-08;15 条收 14 拒 1)** | 交集三条全采纳:①**快照审计**(planner `access:"write"` 而 run-dir 在 gitignore 下、洁净树照不到——开 planner 前后哈希比对真理源+validation/,`planning/` 以外变化即作废恢复重开);②**`planner:produced` 事件**(planner 成默认第四角色后零留痕不可审计——viz 对未知事件安全忽略,渲染后补);③**先小后全判红语义**(配额 AC 小样本散发失败 ≠ 违约,判红须确定性缺陷或 m 按合同 n 不可达+小样本下限 ⌈n/4⌉ 且 ≥3,防把合同允许的随机性当红灯)。另收:planner 挂兜底(连挂 2 次主控有痕降级自行起草)、强制自检未完成 → AC=BLOCKED(NOTES 不承载闸门语义,机器可解析)、K 并发以样本隔离为前提、"转正"术语统一(闸后才叫定稿)。拒 1:planner `return_mode:"ref"`(回复格式本就短,YAGNI) |
| **worktree 模式必须给委托 agent 一份合同只读副本**(dogfood 缺口 2026-07-08) | 真理源 run-dir 在**主树** `.loop/run-<id>/`,而生成者/验证者 `cwd` 是 worktree → **它们读不到合同**。skill 原文只说"cwd 都指向 worktree",漏了这一步,实跑时主控只能临时手动补。定死:建 worktree 后 `cp contract.md` 进 `<worktree>/.loop/run-<id>/`(该 `.gitignore` 同样含 `/.loop/`,不脏树);`amended` 后重新 cp 覆盖。真理源仍在主树,副本只读 |
| **`git clean -fdx` 会不可逆抹掉整个 run-dir**(dogfood 缺口 2026-07-08) | run-dir 在被 gitignore 的 `.loop/` 下,`-x` 连忽略文件一起删 → 合同 + transcript + 面板记录 + 验证者尺子**一并蒸发,无 git 可恢复**(唯一不可逆破坏,由架构师红队在合同闸发现)。三重设防:合同「全局约束」显式禁止 + 三个角色文件内置禁令 + 禁止在 AC/prompt 写"回到干净工作区"这类**诱导措辞**(v1 合同的全局约束 8 正是这么诱导的)。清理未跟踪文件只用 `git clean -fd`(不带 `-x`) |
| **合同闸后「批判的采纳由谁落笔」**(dogfood 缺口 2026-07-08) | skill 只说"你逐条采纳/拒绝留痕",没说改稿人是谁 → 主控容易顺手重写整份合同(= §主控的手 禁止的"顺手起草")。定死:**裁决是主控的**(写 `panel/gate-rulings.md`),**改稿是规划者的**(fresh 出 `contract-draft-v<n>.md`,`planner:produced{kind:"contract"}`——闸前修订仍是草案,`amendment` 留给定稿后;2026-07-08 二次双评订正),主控复核后 cp 覆盖;只有逐字级错字/编号修补允许主控直接改 |
| **合同闸是唯一拦截点:面板须亲手重算每条 AC 期望值 + 主控至少复核一条**(dogfood 实证 2026-07-08) | 实跑中 v1 合同的 `__proto__` AC 期望值与自己的词法规则(A1 ASCII `[A-Za-z0-9]+`,`_` 是分隔符)矛盾:**正确实现必假 FAIL,而唯一含 `_` 的 fixture 让 `\w+` 错误实现恰好通过全部 15 条 AC——合同把"错"设成了唯一通路**。验证者只会照合同判,闸门漏了就整轮跑反。两席异引擎架构师独立收敛于此(高置信),主控 `node` 亲手核验后采纳。故:①面板批判 prompt 必须要求"亲手把每条 fixture 的期望输出算一遍";②主控对判定性 AC 至少亲手复核一条(只读,同「仲裁复跑」档) |
| **验证者执行环境(cwd / fixture 落点)必须写进合同**(dogfood 缺口 2026-07-08) | v1 合同全篇用 `./wordfreq.js`、`f1.txt`、`out1.txt` 相对路径却从不说站在哪 → ①不可执行;②fixture 落产品树,会被主控代 commit 的 `git add`(以 `git status` 为准)**扫进产品提交**。定死:合同钉死 cwd + fixture 一律落 `validation/fixtures/`。修复后实跑:验证者 19 个资产全在 `validation/`,产品树零污染 |
| **不可执行的东西不许伪装成 AC**(dogfood 实证 2026-07-08) | v1 把「禁止 `localeCompare`/`toLocaleLowerCase`」写成受 AC 判定的假设,但架构师实测:A1 把词表锁死在 `[a-z0-9]`,该字母表上 ICU 根排序与 UTF-16 裸比较**处处一致** → 黑盒无论如何判不出来,是条**死条款**。诚实降级为「实现注记」+ 假设账本如实记「本合同验不到它」。**一份合同的价值,一半在于它诚实记录了自己验不到什么** |
...
| **二次双评修复(codex + deepseek 各 NEEDS_FIXES,2026-07-08,针对 13b2935;主控逐条回源核验 + 实验)** | ①Critical:`-x` 禁令可被 **`-X` 绕过**——主控实测 `git clean -fdX` **只删 ignored 文件** → run-dir 精准蒸发、产品树纹丝不动,读起来却像无害的"清理垃圾"(deepseek 此处判"覆盖完整"——它审的是角色覆盖面不是 flag 覆盖面,异引擎双评价值实证)→ 禁令改**禁效果**(任何带 `-x`/`-X` 的 clean + 任何删除 `.loop/` 的操作)+ 检测:事件笔每次 append 都触碰 run-dir,真理源丢失立即 halt;②白名单自相矛盾:13b2935 给主控新增 gate-rulings 落笔与"复核一条 AC"两动作却没进"穷举"白名单,后者还错挂「仲裁复跑」名下(闸阶段无产品可跑)→ 裁决账本并入写真理源行 + 新增「合同闸复算」行(只读演算期望值);③洁净树审计只认 tracked:untracked fixture 逃逸不触发 `val:tainted`,下一轮代 commit 以 porcelain 为准 `git add` 会把它扫进产品提交 → 审计扩为 porcelain **任何输出**都算污染(验证者合法产物全在 ignored 的 validation/ 下,零容忍无误伤);④无人值守中途改约无面板复审,可把降标包装成"修缺陷"自批自过(预算条款恰好开了这条路)→ 收官 broad review 增设**修约审计**职责(独立引擎、必过闸——事后但在交付前,比强拉面板便宜);⑤EVENTS 断线:`contract:confirmed` 补 `rulingsRef` + `by` 语义订正(`by` 记闸的形态,**裁决人恒为主控**,防记假账);闸后修订 kind 订正 `"contract"` + planner 草案**版本化禁覆盖**(重建面板批判的是哪一版,顺带解决 codex 的"contractRef 可变、回放失真");⑥生成者窗口 `validation/` 哈希审计从 YAGNI **转正**:planner 快照审计机制已建、复用近零成本,而被考者改考卷是最强作弊动机(篡改尺子可击穿验证者闸),原"暂不做"理由不再成立。轻处理 2:圆桌 panelist 禁令不跨 skill 加条款(评审席位本就 `read` 档,失败模式表一句话点明);worktree 副本篡改——主控核验发现两评者皆漏:**每轮 send 已内联合同原文**,副本不是判据通道,一句话拆弹 |

**两条致命纪律(内联,同 dev/roundtable)**:`wait` 必传 `timeout_ms`(300000 短轮询循环);别给 `send_message`/`open_session` 传 `wait:true`(超时 abort 掉 turn)。

## 7. 角色文件(`roles/`,通用可 append;2026-07-08 起为三个:generator / validator / planner)
