...
| **合同闸后「批判的采纳由谁落笔」**(dogfood 缺口 2026-07-08) | skill 只说"你逐条采纳/拒绝留痕",没说改稿人是谁 → 主控容易顺手重写整份合同(= §主控的手 禁止的"顺手起草")。定死:**裁决是主控的**(写 `panel/gate-rulings.md`),**改稿是规划者的**(fresh 出 `contract-draft-v<n>.md`,`planner:produced{kind:"amendment"}`),主控复核后 cp 覆盖;只有逐字级错字/编号修补允许主控直接改 |
