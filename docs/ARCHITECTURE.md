...
## v0.11.0 — 委托会话观测台默认开(2026-07-31,**默认行为翻转**)

**变更:** `AGENT_BRIDGE_VIZ` 从「默认关、`=on` 显式开」翻成「**默认开**、`=off` 关」。
每个 MCP server 起来就建一个临时目录,把这次运行的会话与逐轮原文写进去,运行结束整个删掉。

**为什么翻回来。** 2026-07-27 定默认关的理由是隐私,那条**至今成立**:桥的诊断日志刻意不落
prompt 全文,观测台会落 —— 这让「本机磁盘上存在全部委托原文」从不发生变成发生,运行期间
本机任何进程可读。但它**没有解决问题**:记录只在 run 存活期间存在,**出事那次没开就永远查不了**,
而默认关等于把「要不要记」推给一个**当时还没有信息**的人 —— 他得在什么都没发生的时候预判
自己以后会不会需要它。「重度委托之前先记得打开」这句提示,实践中就是不会被想起来。
默认开把决定挪到**有信息**的一侧:明知这次跑敏感内容,才去关。

**判定表刻意不对称**(`scripts/viz-writer.mjs`):未设 / 空 / `on` / `1` / `true` / `yes` → 开;
**其余任何值 → 关**。`off` 打错成 `offf` 时倒向关,因为隐私开关拼错的两个方向代价不一样 ——
少记一次只是查不了,多记一次是内容已经落了盘。

**默认开之后才必须有的两件事**(改默认才暴露出来,各自都是独立缺陷):
- **`VizRecorder` 出生就发一份空快照。** 不发的话「刚开的服、一个会话都还没开」与「快照写了但
  读不出来」在 viewer 眼里完全一样(两个槽都不存在 ⇒ `readLatestState()` 一律返 null),于是
  **刚开服的第一眼就是「暂时读不到记录」+「以下为断连前的最后状态」** —— 一个假故障,而且默认开
  之后人人都会撞上。修在**写的那一侧**,是为了不给 `docs/STATE-session-viz.md` §6.2 那三种
  「看不到内容」再加第四种:状态越多,读的人越要靠猜。
- **开服时扫一遍孤儿观测目录**(`serveMcp()` 里 `createVizRun()` 之前调 `vizCleanup()`)。以前默认关,
  孤儿是罕见事,靠人手跑 `cleanup` 够了;现在**每一次不干净的退出都会留下一份带全部委托原文的目录**。

**同批的页面修复:** 右栏空态拆成两种 —— `renderDetail()` 里 `if(!s)` 一个分支同时管
「一个会话都没有」和「有会话但没选中」,于是左栏列着卡片、右栏却说「还没有可查看的会话」。
两件不同的事走同一个分支,说出来的话就必然对其中一件是假的(与上面那条空快照是同一类错)。

**仓库结构同批整理:** 判据是「这个文件在一次 skill 调用里会不会被打开」。
`docs/repro-mcp-hang/` → `tests/`(它不是文档,也早就不只是那次 MCP 卡死的复现);
两个观测台的页面测试与校验器、三个生成器从 `skills/` 搬到 `tests/` 与 `tools/`;
两份字节合同搬进 `docs/`(`EVENTS-graph.md` / `STATE-session-viz.md`)。
⚠️ loop 与 roundtable 的 `EVENTS.md` **留在各自 skill 目录**:那两个 skill 的事件是**主控 agent
自己 append 的**,跑的时候要照着写 —— 运行时资产,不是开发文档。

**测试:** `tests/repro-viz-writer.mjs` 128 · `tests/repro-viz-bridge.mjs` 75(新增开服孤儿回收,
埋一死一活 —— 只考「死的被删」没有牙,一句无差别批量删除照样全绿)· 真浏览器
`tests/e2e-viz-browser.mjs` 51。四条变异全部验红:默认退回关 / 认不出的值当开 / 不发空快照 / 不扫孤儿。
...
**边界:** 只作用于诊断 `.log` / exit-journal。`answerFile` / `textRef` / MCP 返回的 `text` 仍是**完整未截断全文**——产品契约不变。回归见 `tests/repro-log-bounds.mjs`(74/74)。
