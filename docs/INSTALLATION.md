...
本仓库同时是一个 Claude Code 插件、并自带插件 marketplace，所以一条安装命令即可同时注册 MCP server **和** skill，无需手动处理路径。在 Claude Code 里执行：
...
插件清单（`.claude-plugin/plugin.json`）通过 `${CLAUDE_PLUGIN_ROOT}/scripts/agent-bridge.mjs` 启动 server，无论插件装在哪里都能解析。本地开发时可以直接指向工作副本：

```sh
claude --plugin-dir "$PWD"
```

之后用 `/plugin` 管理（启用/停用/卸载）。后端 agent（`omp`、`codex`）仍需先装好（见第 1 节）。如果只想注册 MCP server（不要插件 / skill），用下面 5.2 或 5.3。
