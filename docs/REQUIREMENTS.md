...
- OMP/Codex session 必须显式关闭，避免后台进程残留。
...
- SSE 实时事件流：向浏览器推送 session 状态变化、OMP text_delta、Codex 流式增量中的 assistant 可见文本。
- 进程清理：记录 OMP/Codex 子进程，跳过仍由 MCP/daemon 拥有的进程，清理 stale records。
...
- 提供 SSE 实时事件流，用于向浏览器推送 OMP/Codex 的状态和输出。
...
- 打开 UI 后能看到 daemon 内已有 OMP/Codex sessions。
- OMP `text_delta` 或等价事件能实时追加到页面。
- Codex `item/agentMessage/delta` 等增量事件能实时追加到页面。
